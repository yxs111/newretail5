...
public class SwaggerConfig {
    ...
    private ApiInfo createAI(){
        return new ApiInfoBuilder().title("新零售平台数据接口").description("基于SpringCloud实现的新零售微服务项目").version("0.1")
    ...
    public Docket createD(){
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(createAI()).select().apis
                (RequestHandlerSelectors.basePackage("com.feri.shop.newretail.api.api")).build();
    }
}
